...
export const combineTransactions = (
  ...
  erc20Transactions.forEach((transaction) => {
    ...
    hashToCombinedTransaction[transaction.hash].erc20Transactions.push(
      transaction,
    );
  });
  ...
  internalTransactions.forEach((transaction) => {
    ...
    hashToCombinedTransaction[transaction.hash].internalTransactions.push(
      transaction,
    );
  });

  // Set the status based on the transactions
  const hashToCombinedTransactionWithType = Object.values(
    hashToCombinedTransaction,
  ).map((combinedTransaction) => {
    return {
      ...combinedTransaction,
  ...
  const migrationTransactions = makeMigrationTransactions(
    address,
    sortedTransactions,
  );
  const transactionsWithMigrations = [
    ...sortedTransactions,
