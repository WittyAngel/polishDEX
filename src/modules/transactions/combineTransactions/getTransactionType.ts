...
export const getTransactionType = (transaction: any) => {
  ...
  const hasTransactionValue =
    transaction.transaction && transaction.transaction.value !== '0';

  const hasInternalIncomingValue =
    transaction.internalTransactions.filter(
      (txn: any) => txn.value !== '0' && isSameAddress(txn.to, address),
    ).length > 0;
  const hasInternalOutgoingValue =
    transaction.internalTransactions.filter(
      (internalTxn: any) =>
        internalTxn.value !== '0' && isSameAddress(internalTxn.from, address),
    ).length > 0;

  const hasValue = hasTransactionValue || hasInternalIncomingValue;
  const isReceivingTransactionEther =
    hasValue &&
    transaction.transaction?.to.toLocaleLowerCase() ===
      address.toLocaleLowerCase();

  const isReceivingInternalEther = hasInternalIncomingValue;

  const isReceivingEther =
    isReceivingTransactionEther ||
    (isReceivingInternalEther && !hasTransactionValue);

  const isSendingEther =
    hasValue &&
    transaction.transaction?.from.toLocaleLowerCase() ===
      address.toLocaleLowerCase();
