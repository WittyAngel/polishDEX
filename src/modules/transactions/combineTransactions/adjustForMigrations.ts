 ...
 */
export const adjustForMigrations = (transactions: any): any => {
  return transactions.map((transaction: any) => {
    if (transaction.erc20Transactions.length === 0) {
      return transaction;
    }

    const newERC20Transactions = transaction.erc20Transactions.map(
      (erc20Transaction: any) => {
        const tokenMigration = tokenMigrations.find(
          (migration) =>
            migration.oldToken.contractAddress ===
              erc20Transaction.contractAddress ||
            migration.newToken.contractAddress ===
              erc20Transaction.contractAddress,
        );
        ...
        const isOldToken =
          tokenMigration.oldToken.contractAddress ===
          erc20Transaction.contractAddress;
        const isTransactionAfterMigration = dayjs(transaction.date).isAfter(
          tokenMigration.date,
        );
