/*eslint-disable*/
const API_KEY = 'ER835NHR17PQJ7R9ZCU3XYFCK99HC6PGXG'; //'BC27S5E5YEK2HDKUNPR6U9R5VV3S15XXAP';
...
type ApiResponse<Result extends unknown> =
...
export const etherScanGet = async <Result extends unknown>(
  ...
  if (result.status === ApiResponseStatus.NOTOK) {
    throw new Error(
      `${typeof result.result === 'string' ? result.result : result.message}`,
    );
  }
