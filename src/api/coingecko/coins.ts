...
export const getAllCoins = () =>
  coinGeckoGet<Coin[]>('/coins/list', { include_platform: true });
...
export const getCoinMarketPrices = (id: string, from: Date) => {
  const fromString = dayjs(from).format('DD-MM-YYYY');

  return coinGeckoGet(`/coins/${id}/history?date=${fromString}`);
};
